...
// ═══ PLAYING SCREEN ════════════════════════════════════════════════════════════
function PlayingScreen({gd,pid,user,gameId,onRoll,onEventResult,eventField,rolling,diceVals,bursts,notif,coins,onBuyItem}){
...
export default function BoardGame({user}){
  ...
  const leaveGame=async()=>{
    if(gameId&&gd){
      ...
      if(gd.host===pid)await remove(ref(db,`games/${gameId}`));
    }
    ...
    const iv=setInterval(async()=>{
      ...
      if(count>14){
        ...
        if(hasExtraDice){
          ...
          // Remove the card
          const newCards=(myData.cards||[]).filter((c,i)=>i!==myData.cards.indexOf("extraDice"));
          await update(ref(db,`games/${gameId}/players/${pid}`),{cards:newCards});
          notify(`🎲 Dupla dobás! ${r2} vs ${roll>r2?roll:r2} → ${roll}`,"#E67E22");
        }
  ...
  return <PlayingScreen gd={gd} pid={pid} user={user} gameId={gameId} onRoll={rollDice} onEventResult={handleEvent} eventField={eventField} rolling={rolling} diceVals={diceVals} bursts={bursts} notif={notif} coins={myCoins} onBuyItem={buyItem}/>;
}
