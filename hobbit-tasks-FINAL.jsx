...
const FAKE_PLAYERS=[{name:"Legolas",race:"elf",score:2840,tasks:15},{name:"Gandalf",race:"wizard",score:2650,tasks:15},{name:"Thorin",race:"dwarf",score:2100,tasks:13},{name:"Aragorn",race:"human",score:1950,tasks:12},{name:"Frodo",race:"hobbit",score:1600,tasks:11},{name:"Bilbo",race:"hobbit",score:1400,tasks:10},{name:"Gimli",race:"dwarf",score:1100,tasks:9},{name:"Elrond",race:"elf",score:800,tasks:7}];
...
function ProfileTab({user,completed,scores}){
  const race=RACES.find(r=>r.id===user?.race)||RACES[3];
  const totalScore=Object.values(scores).reduce((a,b)=>a+b,0);
  ...
  const addFriend=()=>{const name=search.trim();if(!name){setSearchMsg({ok:false,t:"Írj be egy nevet!"});return;}if(name.toLowerCase()===user?.adventureName?.toLowerCase()){setSearchMsg({ok:false,t:"Magadat nem adhatod hozzá!"});return;}if(friends.find(f=>f.name.toLowerCase()===name.toLowerCase())){setSearchMsg({ok:false,t:"Már a barátod!"});return;}const newF={name,race:RACES[Math.floor(Math.random()*RACES.length)].id,score:Math.floor(Math.random()*1800)+200,added:Date.now(),invited:false};const next=[...friends,newF];setFriends(next);localStorage.setItem("hobbit_friends",JSON.stringify(next));setSearch("");setSearchMsg({ok:true,t:`${name} hozzáadva! ⚔️`});setTimeout(()=>setSearchMsg(null),2500);};
  const removeFriend=(name)=>{const next=friends.filter(f=>f.name!==name);setFriends(next);localStorage.setItem("hobbit_friends",JSON.stringify(next));};
  const inviteFriend=(name)=>{const next=friends.map(f=>f.name===name?{...f,invited:true}:f);setFriends(next);localStorage.setItem("hobbit_friends",JSON.stringify(next));setTimeout(()=>{setFriends(f=>f.map(x=>x.name===name?{...x,invited:false}:x));},3000);};
  ...
  const leaderboard=[...FAKE_PLAYERS,{name:user?.adventureName||"Te",race:user?.race||"human",score:totalScore,tasks:completed.length,isMe:true}].sort((a,b)=>b.score-a.score);
    ...
    {tab==="friends"&&<div style={{padding:"14px 12px",display:"flex",flexDirection:"column",gap:12}}>
      <div style={{padding:"12px",background:"rgba(201,168,76,.03)",border:"1px solid rgba(201,168,76,.1)"}}>
        ...
        <div style={{display:"flex",gap:7}}>
          <input value={search} onChange={e=>setSearch(e.target.value)} onKeyDown={e=>e.key==="Enter"&&addFriend()} placeholder="Kalandor neve..." style={{flex:1,background:"rgba(0,0,0,.4)",border:"1px solid rgba(201,168,76,.18)",color:"var(--text)",fontFamily:"'EB Garamond',serif",fontSize:".9rem",padding:"7px 10px",outline:"none"}}/>
          <button onClick={addFriend} style={{padding:"7px 14px",background:"rgba(201,168,76,.1)",border:"1px solid rgba(201,168,76,.35)",color:"var(--gold)",fontFamily:"'Cinzel',serif",fontSize:".65rem",cursor:"pointer"}}>+ Add</button>
        </div>
      ...
      {leaderboard.map((p,i)=>{
        ...
        return <div key={p.name+i} style={{display:"flex",alignItems:"center",gap:9,padding:"9px 10px",background:p.isMe?"rgba(201,168,76,.07)":"rgba(255,255,255,.02)",border:`1px solid ${p.isMe?"rgba(201,168,76,.3)":"rgba(201,168,76,.07)"}`,boxShadow:p.isMe?"0 0 14px rgba(201,168,76,.1)":"none"}}>
          ...
          <div style={{flex:1,minWidth:0}}>
            ...
            <div style={{fontFamily:"'Cinzel',serif",fontSize:".5rem",color:pr_rank.color}}>{pr_rank.label}</div>
          </div>
